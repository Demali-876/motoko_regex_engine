...
module {
  ...
  public class Compiler() {
    ...
    // Helper function to flatten the AST into a sequence of simple nodes
    private func flattenAST(ast: Types.ASTNode): [Types.ASTNode] {
      switch (ast) {
    ...
    switch(alternatives.size()) {
        ...
        case _ {
            ...
            for (level in Iter.range(0, maxLength - 1)) {
                ...
                // Process each alternative at this level
                for (altNodes in flattenedAlts.vals()) {
                    if (level < altNodes.size()) {
                        // Handle node at this level
                        ...
                        // This alternative is shorter, mark current state as accepting
                        acceptStates.add(currentState);
                    };
                };

                // Add all transitions for this level
            ...
            // Final accept states for alternatives that used all levels
            acceptStates.add(currentState);
            ...
            for (i in Iter.range(0, exprs.size() - 2)) {
                switch(buildNFA(exprs[i], currentState)) {
                    ...
                    case (#ok(transitions, accepts)) {
                        ...
                        // Use the last accept state as the next start state
                        currentState := accepts[accepts.size() - 1];
                    };
                };
            };

            // Handle the last expression separately
