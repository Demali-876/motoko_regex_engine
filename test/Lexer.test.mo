...
let tokenizeCases : [(Text, Result.Result<[Types.Token], Types.RegexError>)] = [
  ...
  ("^[^/]*\\.txt$", #ok([{ position = #Instance(0); tokenType = #Anchor(#StartOfString); value = "^" }, { position = #Instance(4); tokenType = #CharacterClass(true, [#Single('/')]); value = "[^/]" }, { position = #Instance(5); tokenType = #Quantifier({ min = 0; max = null; mode = #Greedy }); value = "*" }, { position = #Instance(7); tokenType = #Character('.'); value = "\\." }, { position = #Instance(8); tokenType = #Character('t'); value = "t" }, { position = #Instance(9); tokenType = #Character('x'); value = "x" }, { position = #Instance(10); tokenType = #Character('t'); value = "t" }, { position = #Instance(11); tokenType = #Anchor(#EndOfString); value = "$" }])),
